 ...
 * 精确乘法
 */
function multiply(...nums: numType[]): number {
  if (nums.length > 2) {
    return iteratorOperation(nums, multiply);
  }
 ...
 * 精确加法
 */
function add(...nums: numType[]): number {
  if (nums.length > 2) {
    return iteratorOperation(nums, add);
  }
 ...
 * 精确减法
 */
function sub(...nums: numType[]): number {
  if (nums.length > 2) {
    return iteratorOperation(nums, sub);
  }
...
function round(num: numType, ratio: number): number {
  ...
  let result = divide(Math.round(Math.abs(multiply(num, base))), base);
  if (num < 0 && result !== 0) {
...
// export { strip, add, sub, multiply, divide, round, digitLength, float2Fixed, enableBoundaryChecking };
export {
  math
};
